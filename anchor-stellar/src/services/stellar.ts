...
class StellarService {
   ...
   * - Base reserve: 1 XLM (2 × 0.5 for account to exist)
   * - Per entry (trustline, offer, etc.): 0.5 XLM
   *
   * We use 2 XLM = 1 base + 0.5 trustline + 0.5 margin
   */
  private readonly MIN_ACCOUNT_BALANCE = '2';
  ...
  async activateAccount(userPublicKey: string): Promise<{
    ...
    try {
      ...
      if (exists) {
        ...
        const balances = await this.getAccountBalances(userPublicKey);
        const minRequired = parseFloat(this.MIN_ACCOUNT_BALANCE);

        if (balances.xlm < minRequired) {
          const topUpAmount = (minRequired - balances.xlm + 0.5).toFixed(7); // Add extra margin
          console.log(`[Stellar] Account needs top-up: ${balances.xlm} XLM -> sending ${topUpAmount} XLM`);

          const sponsorAccount = await horizon.loadAccount(this.sponsorKeypair.publicKey());
          const paymentTx = new TransactionBuilder(sponsorAccount, {
            fee: '100000',
          ...
          paymentTx.sign(this.sponsorKeypair);
          const result = await horizon.submitTransaction(paymentTx);
          console.log(`[Stellar] Top-up sent! Hash: ${result.hash}`);
        }
