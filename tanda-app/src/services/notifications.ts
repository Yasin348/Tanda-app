 ...
 *
 * Features:
 * - Deposit reminders (3 days, 1 day, due date)
 * - Payout received notifications
 * - Tanda started notifications
 * - Member joined notifications
 * - Expulsion notifications
 * - Lockout management for failed attempts
 */
...
class NotificationService {
  ...
  /**
   * Send tanda started notification
   */
  async sendTandaStartedNotification(tandaName: string): Promise<void> {
    if (Platform.OS === 'web') return;

    try {
      await Notifications.scheduleNotificationAsync({
        content: {
          title: 'Tanda iniciada',
          body: `La tanda "${tandaName}" ha comenzado. Realiza tu primer deposito!`,
          data: { type: 'tanda_started' },
          sound: 'default',
          ...(Platform.OS === 'android' && { channelId: CHANNEL_ID }),
        },
        trigger: null,
      ...
      console.error('[Notifications] Error sending tanda started notification:', error);
    }
  }
    ...
    try {
      await Notifications.scheduleNotificationAsync({
        content: {
          ...
          body: memberName
            ? `${memberName} se ha unido a "${tandaName}"`
            : `Alguien se ha unido a tu tanda "${tandaName}"`,
      ...
      console.error('[Notifications] Error sending member joined notification:', error);
    }
  }

  /**
   * Schedule a deposit reminder for a specific time
   */
  async scheduleDepositReminder(
    tandaId: string,
    tandaName: string,
    amount: number,
    reminderDate: Date
    ...
    try {
      const identifier = await Notifications.scheduleNotificationAsync({
        content: {
          title: 'Deposito pendiente',
          body: `Recuerda depositar €${amount.toFixed(2)} en "${tandaName}"`,
          data: { tandaId, type: 'deposit_reminder' },
          sound: 'default',
          ...(Platform.OS === 'android' && { channelId: CHANNEL_ID }),
        },
        trigger: {
