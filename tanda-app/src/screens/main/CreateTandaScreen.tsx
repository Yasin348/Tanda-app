...
export const CreateTandaScreen: React.FC<CreateTandaScreenProps> = ({
  ...
  const handleCreate = async () => {
    ...
    if (amountNum < 1) {
      alertMessage('Error', 'El monto minimo es 1 EUR');
      return;
    }
    if (participantsNum < 2 || participantsNum > 12) {
      alertMessage('Error', 'El numero de participantes debe ser entre 2 y 12');
      return;
    }
    const minScoreNum = parseInt(minScore);
    if (isNaN(minScoreNum) || minScoreNum < 0 || minScoreNum > 100) {
      alertMessage('Error', 'La puntuacion minima debe ser entre 0 y 100');
      return;
    }
        ...
        >
          ...
          <View style={styles.commissionInfo}>
            ...
            <View style={styles.commissionTextContainer}>
              <Text style={styles.commissionTitle}>Comision por deposito</Text>
              <Text style={styles.commissionText}>
                {formatEuro(TANDA_CONFIG.commission)} por cada deposito
              </Text>
            </View>
          </View>
        </ScrollView>
