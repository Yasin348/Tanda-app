...
export const MYKOBO_CONFIG = {
  ...
  // Fees for display (actual fees come from Mykobo /sep24/info)
  fees: {
    onramp: 0.5,      // ~0.5% EUR -> EURC
    offramp: 0.5,     // ~0.5% EURC -> EUR
    minAmount: 25,    // Minimum 25 EUR
    maxAmount: 30000, // Maximum 30,000 EUR
  },
};
...
export const FEE_CONFIG = {
  ...
  freeOperations: ['kyc', 'trustline', 'advance'],
};
 ...
 *
 * Commission model (charged by smart contract):
 * - Create tanda: 0.10 EUR fixed fee
 * - Deposit: 0.5% of deposit amount
 * - Advance: FREE (to incentivize users to advance the tanda)
 */
export const TANDA_CONFIG = {
  // Create tanda fee (fixed amount in EURC)
  createTandaFee: 0.10,

  // Deposit commission (percentage, 0.5 = 0.5%)
  depositCommissionPercent: 0.5,

  // Safety fund percentage
  safetyFundPercent: 5,

  // Helper function to calculate deposit commission
  calculateDepositCommission: (amount: number): number => {
