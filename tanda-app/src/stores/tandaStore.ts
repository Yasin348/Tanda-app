...
export const useTandaStore = create<TandaState>((set, get) => ({
  ...
  deposit: async (tandaId: string) => {
    ...
    try {
      ...
      // Commission is 0.5% of deposit amount, charged by smart contract
      ...
      if (userBalance < totalAmount) {
        const errorMsg = `Fondos insuficientes. Necesitas €${totalAmount.toFixed(2)} (€${tanda.amount.toFixed(2)} + ${TANDA_CONFIG.depositCommissionPercent}% comisión) pero solo tienes €${userBalance.toFixed(2)}`;
      ...
      console.log(`[TandaStore] Transferring €${totalAmount.toFixed(2)} (€${tanda.amount.toFixed(2)} + ${TANDA_CONFIG.depositCommissionPercent}% commission) to Storage Account: ${storageAccountAddress}`);
